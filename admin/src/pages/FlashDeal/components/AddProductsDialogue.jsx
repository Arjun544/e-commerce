...
import ProductsDropDown from "../../Banners/components/ProductsDropDown";
...
const AddProductsDialogue = ({
  ...
  useEffect(() => {
    setAddedProducts(currentDeal.products);
    setDiscounts(currentDeal.products.map((item) => item.discount));
  }, []);
  ...
  const handleSave = async (e) => {
    ...
    try {
      setLoading(true);
      await addDealProducts(currentDeal._id, productObject, addedProducts);
      setLoading(false);
      socket.current.on("add-dealProducts", (newBanners) => {
        setDeals(newBanners);
      });
