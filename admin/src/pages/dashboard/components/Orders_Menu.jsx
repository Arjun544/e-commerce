...
const OrdersMenu = ({ isOrderMenuOpen, setIsOrderMenuOpen }) => {
  ...
  return (
    <div className="flex absolute z-40 right-0 w-full h-full backdrop-filter backdrop-blur-md">
      <div className="flex flex-col absolute top-0 right-0 z-50 w-1/5 h-full">
        <div className="w-full pt-4 pl-5 bg-bgColor-light">
          <XIcon
            onClick={(e) => {
              e.preventDefault();
              setIsOrderMenuOpen(false);
        ...
        <div className="flex flex-col items-start h-full px-5 pt-4 bg-bgColor-light overflow-y-auto scrollbar scrollbar-thin hover:scrollbar-thumb-gray-900 scrollbar-thumb-gray-500 scrollbar-track-gray-300">
          {orders.filter((items) =>
            ...
            <div className="h-full w-full flex items-center justify-center">
              <span className="font-bold tracking-wider text-gray-400">
                No Orders
              </span>
            </div>
          ) : (
            orders
              .filter((items) =>
                moment(
                  new Date(new Date(items.dateOrdered).getTime())
                ).isBetween(
                  moment(new Date(new Date().getTime() - 24 * 60 * 60 * 1000))
                )
              )
              .map((order, index) => (
                <div
                  ...
                  className="flex items-center justify-between py-4 w-full mb-4 bg-white rounded-3xl px-4 shadow-sm cursor-pointer  hover:shadow-md"
                >
                  <div className="flex">
                    {order.user.profile ? (
                      <img
                        className="w-12 h-12 rounded-2xl mr-3  object-cover"
